...
public class TransactionService {
    ...
    public void notifyNextReservation(int bookId) {
    	List<Reservation> reservations= reservationRepository.findByBook_IdAndStatus(bookId, ReservationStatus.PENDING);
    	 if (!reservations.isEmpty()) {
    	        Reservation nextReservation = reservations.get(0);
    	        nextReservation.setStatus(ReservationStatus.COMPLETED);
    	        reservationRepository.save(nextReservation);
    	        
    	        // Notify the student (you could add email notifications or other logic here)
    	        System.out.println("Notified student with ID: " + nextReservation.getStudent().getId() + " that the book is available.");
    	    }
    }
